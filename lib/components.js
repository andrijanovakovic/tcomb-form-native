...
class Textbox extends Component {
  ...
  getLocals() {
    ...
    locals.inputRef = this.setInputRef;
...
class Select extends Component {
  ...
  getLocals() {
    ...
    locals.inputRef = this.setPickerRef;
...
class Struct extends Component {
  ...
  isValueNully() {
    return Object.keys(this.inputs).every(
      key => this.inputs[key].isValueNully()
    );
  }

  removeErrors() {
    this.setState({ hasError: false });
    Object.keys(this.inputs).forEach(key => this.inputs[key].removeErrors());
  }

  getValue() {
    const value = {};
    for (const key in this.inputs) {
        value[key] = this.inputs[key].getValue();
    }
  ...
  validate() {
    ...
    for (const key in this.inputs) {
      result = this.inputs[key].validate();
  ...
  getComponent(path) {
    ...
    const child = this.inputs[head];
    if (!child) {
      return null;
    }
    if (tail.length === 0 || !child.getComponent) {
      return child;
    }
    return child.getComponent(tail);
  }
}
...
export class List extends Component {
  ...
  removeErrors() {
    this.setState({ hasError: false });
    Object.keys(this.items).forEach(key => this.items[key].removeErrors());
  }
  ...
  getValue() {
    ...
    for (let i = 0, len = this.state.value.length; i < len; i++) {
      if (this.items[i]) {
        value.push(this.items[i].getValue());
      }
    }
    return this.getTransformer().parse(value);
  }
...
class Form extends React.Component {
  ...
  pureValidate() {
    if (!this.input || !this.input.pureValidate) {
      return new t.ValidationResult({ errors: [], value: null });
    }
    return this.input.pureValidate();
  }
  ...
  getComponent(path) {
    const arr = t.String.is(path) ? path.split(".") : path;
    if (!this.input || !this.input.getComponent) {
      return null;
    }
    return this.input.getComponent(arr);
  }
